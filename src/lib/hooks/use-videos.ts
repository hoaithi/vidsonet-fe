...
export const useVideos = () => {
  ...
  // ✅ User Reaction State (NEW FORMAT)
  ...
  const checkUserReaction = async (videoId: string, userId?: string) => {
    try {
      ...
      // Transform data to match VideoInfoActions expectations
      if (response.result && response.result.hasReacted) {
        setUserReaction({
          hasReacted: true,
          reactionType: response.result.reactionType
        });
        console.log("✅ User reaction loaded:", response.result.reactionType);
      } else {
        setUserReaction({
          hasReacted: false,
          reactionType: undefined
  ...
  // ✅ Reset User Reaction (FIXED)
  const resetUserReaction = () => {
    setUserReaction({
      hasReacted: false,
      reactionType: undefined
    });
    console.log("✅ User reaction reset");
  };
