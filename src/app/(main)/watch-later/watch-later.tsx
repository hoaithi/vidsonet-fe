...
export default function WatchLater() {
  ...
  const { isLoading, removeFromWatchLater } =
    usePlaylists(); 
    const {getWatchLaterVideos} = useVideos();  
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      ...
      if (watchLaterVideos) setVideos(watchLaterVideos.content);
    };
  ...
  return (
    <div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground mb-6">
            Save videos to watch later by clicking the Watch Later button on
            videos.
          </p>
      ...
      <AlertDialog
        open={isRemoving}
        onOpenChange={(open) => !open && setIsRemoving(false)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Remove from Watch Later</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to remove this video from your Watch Later
              list?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={confirmRemoveVideo}>
              Remove
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
